...
if __name__=="__main__":
    ...
    for i in range(config["job_num"]):
        submission_scripts.append("submit_mach3_JointAtmFit_{}.lsf".format(i))
        run_scripts_job=[]
        start_id = i * num_rs_job * config["chain_per_rs"]
        for j in range(num_rs_job):
            run_scripts_job.append("run_mach3_JointAtmFit_{0}_{1}.sh".format(start_id,start_id+config["chain_per_rs"]-1))
    ...
    for i,run_scripts_job in enumerate(run_scripts_tot):
        ...
        for j,run_script in enumerate(run_scripts_job):
            ...
            for chain_id in chain_ids:
                ...
                os.system(sed_command)
                sed_command="sed -i -e '/^#CHAINS/a ./AtmJointFit_Bin/JointAtmFit ${storage_dir}/chain_"+str(chain_id)+"/config/AtmConfig.cfg &> ${storage_dir}/chain_"+str(chain_id)+"/output/jointAtmFit.log &' "+run_script
                os.system(sed_command)
    ...
    for cid in chains:
        ...
        os.system("cp "+work_dir+"/AtmConfig.cfg config/")
        #change configs
        os.chdir("config")
        sed_command="sed -i 's|NSTEPS.*|NSTEPS = "+str(config["num_step"])+"|' AtmConfig.cfg"
        os.system(sed_command)    
        sed_command="sed -i 's|OUTPUTNAME.*|OUTPUTNAME = \""+config["out_dir"]+"/job_1/chain_"+str(cid)+"/output/MaCh3-Atmospherics-MCMC.root\"|' AtmConfig.cfg"
        os.system(sed_command)
        if config["use_bb"]:
            sed_command="sed -i 's|ATMCONFIGDIR.*|ATMCONFIGDIR = \""+config["out_dir"]+"/job_1/SampleConfigs/SampleConfigs_bb\"|' AtmConfig.cfg"
            os.system(sed_command)
            sed_command="sed -i 's|BEAMCONFIGDIR.*|BEAMCONFIGDIR = \""+config["out_dir"]+"/job_1/SampleConfigs/SampleConfigs_bb\"|' AtmConfig.cfg"
            os.system(sed_command)
        else: 
            sed_command="sed -i 's|ATMCONFIGDIR.*|ATMCONFIGDIR = \""+config["out_dir"]+"/job_1/SampleConfigs\"|' AtmConfig.cfg"
            os.system(sed_command)
            sed_command="sed -i 's|BEAMCONFIGDIR.*|BEAMCONFIGDIR = \""+config["out_dir"]+"/job_1/SampleConfigs\"|' AtmConfig.cfg"
            os.system(sed_command)
